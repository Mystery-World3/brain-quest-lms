...
import { listenToLeaderboard, getQuizzes } from '@/services/database';
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    const lastResultRaw = localStorage.getItem('last_result');
    ...
    const loadQuizAndLeaderboard = async () => {
      try {
        ...
        const foundQuiz = quizzes.find((q: any) => q.id === parsed.quizId) || quizzes[0];
        setQuiz(foundQuiz);

        const unsubscribe = listenToLeaderboard(parsed.quizId, parsed.classId, (scores) => {
          const mappedScores = scores.map((s: any) => ({
            ...s,
            isCurrent: s.name === name && s.score === parsed.score
          }));
          setClassScores(mappedScores);
        });
  ...
  const currentClassName = result.className || "Kelas";

  return (
    <div className="min-h-screen bg-background p-4 md:p-8 flex flex-col items-center animate-in fade-in duration-1000">
      ...
      <div className="w-full max-w-4xl space-y-10 pb-40">
        <div className={cn(
          "relative overflow-hidden rounded-[2rem] md:rounded-[3rem] p-8 md:p-20 shadow-2xl text-center border-4 md:border-8 border-white dark:border-primary/20 animate-in zoom-in-95 duration-1000",
          isHighPerformance 
          ...
          <div className="absolute -top-16 -right-16 opacity-10 rotate-12 hidden md:block animate-pulse">
            <Trophy size={300} />
          </div>
          <div className="relative z-10 flex flex-col items-center">
            ...
            <h1 className="text-4xl md:text-7xl font-headline font-black mb-4 md:mb-6 drop-shadow-2xl animate-in slide-in-from-top-8 duration-700">
              {isHighPerformance ? 'LUAR BIASA!' : 'KERJA BAGUS!'}
            </h1>
            <p className="text-white/95 text-lg md:text-2xl font-bold mb-8 md:mb-12 max-w-2xl mx-auto leading-relaxed animate-in fade-in duration-1000 delay-300">
              {isHighPerformance 
                ? 'Prestasi yang membanggakan! Pertahankan semangat belajarmu untuk tantangan berikutnya.'
                : 'Kamu sudah mencoba yang terbaik! Mari kita ulas kembali jawabanmu.'}
            </p>
            
            <div className="relative group">
              <div className="absolute inset-0 bg-white/40 blur-3xl rounded-full scale-125 animate-pulse" />
              <div className="relative inline-flex flex-col items-center justify-center bg-white text-primary rounded-full w-44 h-44 md:w-56 md:h-56 shadow-[0_15px_60px_-15px_rgba(255,255,255,0.5)] border-[10px] md:border-[15px] border-primary/10 transform hover:scale-110 transition-transform duration-500">
        ...
        <div className="flex justify-center px-4 animate-in slide-in-from-bottom-8 duration-700 delay-500">
           <Card 
            onClick={() => setShowScoreboard(true)}
            className="w-full max-w-md p-6 md:p-8 flex flex-row items-center gap-4 md:gap-6 border-4 rounded-[2rem] md:rounded-[2.5rem] hover:bg-accent/5 transition-all cursor-pointer group shadow-xl border-accent/10 glass-morphism active:scale-95"
           >
              ...
              <ArrowRight className="ml-auto text-accent group-hover:translate-x-2 transition-transform" />
           </Card>
        </div>

        <div className="space-y-6 md:space-y-8 px-4">
          <h2 className="text-2xl md:text-3xl font-headline font-black flex items-center gap-4 animate-in slide-in-from-left-4 duration-500">
            ...
            {quiz.questions.map((q: any, i: number) => {
              ...
              return (
                <Card key={q.id} className={cn(
                  "border-4 rounded-[2rem] md:rounded-[2.5rem] shadow-xl transition-all duration-300 student-card-hover overflow-hidden animate-in fade-in slide-in-from-bottom-4",
                  ...
                  <div className="p-6 md:p-8 flex flex-col gap-6 md:gap-8">
                    <div className="flex flex-col md:flex-row gap-6 md:gap-8">
                      <div className={cn(
                        "w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl shrink-0 flex items-center justify-center font-black text-xl md:text-2xl shadow-sm transform transition-all group-hover:rotate-6",
                        isCorrect 
                      ...
                      <div className="flex-1 space-y-4 md:space-y-6">
                        <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                          ...
                          {isCorrect ? (
                            <Badge className="bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300 border-none shrink-0 py-2 px-4 rounded-xl font-black text-xs tracking-widest animate-pulse">
                              <CheckCircle2 size={16} className="mr-2" /> BENAR
                            </Badge>
                          ) : (
                            <Badge className="bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-300 border-none shrink-0 py-2 px-4 rounded-xl font-black text-xs tracking-widest animate-pulse">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className={cn(
                            "p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border-2 flex flex-col gap-2 shadow-inner transition-colors",
                            isCorrect 
                            ...
                            <span className="text-base md:text-lg font-black">
                              {q.type === 'multiple-choice' 
                                ...
                                : (result.answers[i] === -1 || result.answers[i] === '' ? 'Kosong' : result.answers[i])
                              }
                            </span>
                          </div>
                          
                          {!isCorrect && (
                            <div className="p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border-4 border-green-500/20 bg-green-50/30 dark:bg-green-950/10 flex flex-col gap-2 shadow-inner animate-in slide-in-from-right-4 duration-500">
                    ...
                    {q.explanation && (
                      <div className="mt-2 md:mt-4 bg-muted/30 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] border-2 border-dashed border-muted-foreground/20 group hover:border-primary/30 transition-all">
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 md:p-8 bg-background/80 backdrop-blur-2xl border-t-4 border-primary/5 flex justify-center gap-4 md:gap-6 z-50 animate-in slide-in-from-bottom-full duration-700">
          <Button 
            variant="outline" 
            onClick={() => router.push('/')} 
            className="h-14 md:h-16 px-6 md:px-10 rounded-xl md:rounded-2xl font-black text-lg md:text-xl border-4 transition-all hover:scale-105 active:scale-95 shadow-xl bg-card"
          >
      ...
      <Dialog open={showScoreboard} onOpenChange={setShowScoreboard}>
        <DialogContent className="max-w-2xl w-[95vw] rounded-[2rem] md:rounded-[3rem] p-0 overflow-hidden border-none shadow-2xl bg-background animate-in zoom-in-95 duration-500">
          ...
          <div className="p-4 md:p-8 space-y-4 max-h-[50vh] overflow-auto bg-background/50 backdrop-blur-sm">
            {classScores.length > 0 ? classScores.map((score, idx) => (
              <div 
                key={idx}
                className={cn(
                  "flex items-center justify-between p-4 md:p-6 rounded-2xl border-4 transition-all duration-300 animate-in slide-in-from-bottom-4",
                  score.isCurrent 
                ...
                <div className="flex items-center gap-4 md:gap-6">
                  <div className={cn(
                    "w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center font-black text-lg md:text-xl shadow-sm transform hover:rotate-6 transition-transform",
          ...
          <div className="p-6 md:p-8 bg-muted/20 border-t flex justify-center">
            <Button onClick={() => setShowScoreboard(false)} className="h-12 md:h-14 px-10 md:px-12 rounded-xl md:rounded-2xl font-black text-lg shadow-xl active:scale-95 transition-all w-full sm:w-auto bg-primary text-white">
              Tutup Papan Skor
            </Button>
          </div>
        </DialogContent>
